...
void open_file(gchar *filename)
{
	...
	if (g_file_get_contents(filename, &contents, NULL, NULL))//La fonction étant un boolean, il retourne soit TRUE ou FALSE, s'il est TRUE ça veut dire que l'opération pour récupérer le contenu du fichier a été réalisé avec succès
	{

...
/*int findIndexToInsert(double *tab, Base * a, int length, int *l);
void shiftRightAt(int index, int length, double *tab, Base * a, int *l);*/
void initmin(Base *a, double * list, double *listmin, int length);
void initmax(Base *a, double * list, double *listmax, int length);
/*int length(Base * a);
int length2(Base * a);
int find(Base * a, double * list, int length);*/
/*void quiz2(Base *a, int *choix);*/
/* quiz3(Base *a, int *choix);
void quiz4(Base * a);*/
...
/*void quiz(Base *a)
{
   int i=0, j=0, k=0; char entry[30];
   printf("Bien vous avez entré %d.", a->numword);
   if(a->numword!=0)
   {
     printf(" Vous aurez maintenant un quiz sur chaque mot que vous avez entré.\n");
     do
     {
       printf("Quelle est la traduction du mot ");
       while(a->mots[i][j]!='\0')
...
/*int length2(Base * a)
{
 int i=0, j=0;
  for(i=0; i<a->numword; i++)
  {
   if(a->winratetab[i]>20 && a->winratetab[i]!=900)
   {
    j++;
   }
  }
...
{
  srand(time(NULL)); 
  int j=0, i=0, k=0, l=0, max=length-1, max2=a->numword-1, min=0;k=rand()%(max2+1-min)+min;
    while(l==0)
    {
    i=rand()%(max+1-min)+min;
    if(list[i]!=900)
    {
    l++;
    while(j==0 && k<a->numword)
    {
    if(list[i]==a->winratetab[k] && a->winratetab[k]!=900)
    {
      j++;
      list[i]=900;
    }
...
/*void quiz2(Base *a, int *choix)
{
  int i=0, j=0, k=0, l=0, index=0, length=10; double listmin[length]; char entry[30];
  initmin(&*a, listmin);
  tribulle(listmin, length);
  if(a->numword>10)
  {
    for(l=10; l<a->numword; l++)
    {
     index=findIndexToInsert(listmin, &*a, length, &l);
     if(index<length)
     {
       shiftRightAt(index, length, listmin, &*a, &l);
     }
    }
  }
  l=0;
     printf("Bien. Vous allez donc être interoggé sur les 10 mots les moins maîtrisés.\n");
     do
     {
       printf("Quelle est la traduction du mot ");
       i=findmin(&*a, listmin, l);
       a->winratetab[i]=900;
       while(a->mots[i][j]!='\0')
       {
        printf("%c", a->mots[i][j]);
       ...
       i=0; j=0; k=0;l++;
     }while(l<length && l<a->numword);  
afficheresult(&*a, &*choix);
}*/

/*void quiz3(Base *a, int *choix)
{
  int i=0, j=0, k=0, l=0, length; double *listmin; char entry[30];
  length=length1(&*a); listmin= malloc( length * sizeof(double));
  initmin(&*a, &*listmin);
  tribulle(&*listmin, length);
     printf("Bien. Vous allez donc être interoggé sur les mots les moins maîtrisés.(Max=10) \n");
     do
     {
       printf("Quelle est la traduction du mot ");
       i=find(&*a, &*listmin, length);
       a->winratetab[i]=900;
       while(a->mots[i][j]!='\0')
       {
        printf("%c", a->mots[i][j]);
...
/*void quiz4(Base * a)
{
  int i=0, j=0, k=0, l=0, length; double *listmax; char entry[30];
  length=length2(&*a);listmax= malloc(length * sizeof(double));
  initmax(&*a, &*listmax);
  tribulle(&*listmax, length);
     printf("Bien. Vous allez donc être interoggé sur les mots les plus maîtrisés.(Max=10)\n");
     do
     {
       printf("Quelle est la traduction du mot ");
       i=find(&*a, &*listmax, length);
       a->winratetab[i]=900;
       while(a->mots[i][j]!='\0')
       {
        printf("%c", a->mots[i][j]);
